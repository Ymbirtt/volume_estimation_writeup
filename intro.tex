...
\Input{$\mathcal{O}_K$, an inclusion oracle for $K$, $n$, the number of dimensions in which $K$ exists, $R$, the radius of $K$, a number of points to use per thread, $n_p$ and an accuracy parameter for the sampling algorithm $\theta$}
...
\Begin{
	...
	$\mathcal{O}_{K_0} <- \top$
	\For{$i <- 1$ \KwTo $n_s$}{
		$\mathcal{O}_{K_i} <-$ make\_oracle $(e^i, \mathcal{O}_K)$ \\
		$n_{in} <- 0$
		\For{$j <- 1$ \KwTo $n_p$}{
			$p <-$ generate\_point$(O_{K_i})$ \\
			\If{$\mathcal{O}_{K_{i-1}}(p)$}{
				$n_{in} <- n_{in} + 1$
			}
		}
		$v <- \frac{n_s v}{n_{in}}$
